    ...
    return BlocConsumer<AtelierBloc, AtelierState>(
      listener: (context, state) {
        if (state.action == AtelierAction.add) {
          successToast(context: context, msg: "Enfant ajouté avec succès");
          AutoRouter.of(context).pop();
        }
        if (state.action == AtelierAction.update) {
          successToast(context: context, msg: "Enfant modifié avec succès");
          AutoRouter.of(context).pop();
        }
        if (state.action == AtelierAction.delete) {
          successToast(context: context, msg: "Enfant supprimé avec succès");
          AutoRouter.of(context).pop();
        }
      },
              ...
              state.vicariatAteliers == null
                  ...
                  : Column(
                    children: [
                      ...
                      Expanded(
                        child:
                            state.vicariatAteliers!.isEmpty
                                ? EmptyWidget(text: 'Aucun enfant trouvé')
                                : ListView.builder(
                                  shrinkWrap: true,
